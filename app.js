...
async function ensureCache() {
  if (_cacheReady && !window.__fbReloadNeeded) return;
  window.__fbReloadNeeded = false;
  _cacheReady = false;
...
function getMonthWeeks(year, month) {
  ...
  while (weeks.length < 7) {
    ...
    if (wStart.getMonth() > month && wStart.getFullYear() >= year) break;
  }
  return weeks;
}
...
async function generateMonthWeeks(monthId) {
  const pages = loadPlanPages();
  const monthPage = pages.find(p => p.id === monthId);
  const existing = pages.filter(p => p.parentId === monthId);

  let year = new Date().getFullYear(), monthIndex = 0;
  if (monthPage) {
    const parts = monthPage.title.split(' ');
  ...
  for (let i = 0; i < weeks.length; i++) {
    ...
    const title = `Неделя ${i + 1}`;
    if (existing.some(p => p.title === title)) continue;
    pages.push({
      id: newPlanPageId(), type: 'week', parentId: monthId, title, order: i,
      dateStart: fmtDate(start), dateEnd: fmtDate(end)
    });
  }
  _cache[PLAN_PAGES_KEY] = pages;
  renderPlansNav();
...
async function generateWeekDays(weekId) {
  const pages = loadPlanPages();
  const weekPage = pages.find(p => p.id === weekId);
  const existing = pages.filter(p => p.parentId === weekId);

  let weekStart = null;
  if (weekPage && weekPage.dateStart) {
    const [d, m] = weekPage.dateStart.split('.').map(Number);
    const monthPage = pages.find(p => p.id === weekPage.parentId);
    let year = new Date().getFullYear();
    if (monthPage) {
      const y = parseInt(monthPage.title.split(' ').pop());
      if (!isNaN(y)) year = y;
    }
    weekStart = new Date(year, m - 1, d);
  }

  for (let d = 0; d < 7; d++) {
    const date = weekStart ? new Date(weekStart.getTime() + d * 86400000) : null;
    const dateStr = date ? ` ${fmtDate(date)}` : '';
    const title = WEEKDAY_NAMES[d] + dateStr;
    if (existing.some(p => p.title === title)) continue;
...
function renderItems(filter = '') {
  ...
  // Checkbox-based rendering for "plans" (Планы)
  if (currentTab === 'plans') {
    emptyState.classList.add('hidden');
    const placeholder = currentPlanPageId ? 'Добавить задачу...' : 'Выбери страницу для добавления задач';
    grid.innerHTML = `
      <div class="tasks-checklist">
        <div class="tasks-input-row">
          <input type="text" id="plans-quick-input" class="tasks-quick-input" placeholder="${placeholder}" ${!currentPlanPageId ? 'disabled' : ''} />
        </div>
        <div class="plans-list"></div>
      </div>
    `;
    renderPlansList();
    const input = document.getElementById('plans-quick-input');
    if (input && !input.disabled) {
      input.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' && input.value.trim()) {
          addQuickPlanItem(input.value.trim());
          input.value = '';
        }
      });
    }
...
async function addQuickPlanItem(title) {
  if (!currentPlanPageId) return;
  const items = getItems('plans');
  items.unshift({
    id: Date.now().toString(36) + Math.random().toString(36).slice(2, 6),
    title,
    desc: '',
    ...
    pageId: currentPlanPageId,
    createdAt: Date.now()
  });
  await saveItems('plans', items);
  renderItems(document.getElementById('search-input')?.value || '');
  renderPlansSidebar();
  // Восстанавливаем фокус на поле ввода после перерисовки
  const input = document.getElementById('plans-quick-input');
...
function renderPlansList() {
  const filter = (document.getElementById('search-input')?.value || '').toLowerCase();
  let items = getItems('plans').filter(i => (i.pageId || null) === currentPlanPageId);
  if (filter) items = items.filter(i => i.title.toLowerCase().includes(filter));
  const listEl = document.querySelector('.plans-list');
  if (!listEl) return;
  if (!items.length) {
    listEl.innerHTML = '<div class="tasks-empty">\u041d\u0435\u0442 \u0437\u0430\u0434\u0430\u0447. \u0414\u043e\u0431\u0430\u0432\u044c \u043f\u0435\u0440\u0432\u0443\u044e!</div>';
    return;
  }
  listEl.innerHTML = items.map(item => `
    <label class="tasks-item ${item.status === 'done' || item.done ? 'is-checked' : ''}" data-id="${item.id}">
      <input type="checkbox" ${item.status === 'done' || item.done ? 'checked' : ''} onchange="togglePlanItemCheck('${item.id}', this.checked)" />
      <span class="tasks-item-text">${escHtml(item.title)}</span>
      <button class="tasks-item-del" onclick="event.preventDefault();deletePlanItem('${item.id}')" title="\u0423\u0434\u0430\u043b\u0438\u0442\u044c">\u2715</button>
    </label>
  `).join('');
  document.getElementById('total-count').textContent = `${totalCount()} \u0437\u0430\u043f\u0438\u0441\u0435\u0439`;
}

async function togglePlanItemCheck(id, checked) {
  const items = getItems('plans');
  const item = items.find(i => i.id === id);
  if (!item) return;
  item.status = checked ? 'done' : 'not_started';
  item.done = checked;
  if (checked) {
    item.doneAt = Date.now();
  }
  ...
  renderItems(document.getElementById('search-input')?.value || '');
  renderPlansSidebar();
  tgHaptic?.(checked ? 'medium' : 'light');
}

async function deletePlanItem(id) {
  const items = getItems('plans').filter(i => i.id !== id);
  await saveItems('plans', items);
  renderItems(document.getElementById('search-input')?.value || '');
  renderPlansSidebar();
}
...
function switchTab(tab) {
  ...
  document.getElementById('open-modal').style.display = (isTasks || isPlans) ? 'none' : '';

  const plansNav = document.getElementById('plans-nav');
  const plansSidebar = document.getElementById('plans-right-sidebar');
  const sidebarTree = document.getElementById('sidebar-plans-tree');
  if (tab === 'plans') {
    currentPlanPageId = null;
    plansNav.classList.remove('hidden');
    plansSidebar && plansSidebar.classList.remove('hidden');
    sidebarTree && sidebarTree.classList.remove('hidden');
    renderPlansNav();
    renderPlansSidebar();
    renderSidebarPlansTree();
  } else {
    plansNav.classList.add('hidden');
    plansSidebar && plansSidebar.classList.add('hidden');
    sidebarTree && sidebarTree.classList.add('hidden');
  }
...
function getPageProgress(pageId) {
  ...
  function collect(pid) {
    const direct = items.filter(i => i.pageId === pid);
    const children = pages.filter(p => p.parentId === pid);
    let total = direct.length, done = direct.filter(i => i.status === 'done' || i.done).length;
    for (const c of children) {
      const sub = collect(c.id);
      total += sub.total; done += sub.done;
    }
    return { total, done };
  }
  return collect(pageId);
}

function getTodayFmtDate() {
  const now = new Date();
  return now.getDate().toString().padStart(2,'0') + '.' + (now.getMonth()+1).toString().padStart(2,'0');
}

function isTodayInWeek(page) {
  if (page.type !== 'week' || !page.dateStart || !page.dateEnd) return false;
  const today = getTodayFmtDate();
  return page.dateStart <= today && today <= page.dateEnd;
}

function isTodayDay(page) {
  if (page.type !== 'day') return false;
  const today = getTodayFmtDate();
  return page.title.includes(today);
}

function goToToday() {
  const pages = loadPlanPages();
  const today = getTodayFmtDate();
  const todayPage = pages.find(p => p.type === 'day' && p.title.includes(today));
  if (todayPage) { selectPlanPage(todayPage.id); return; }
  const todayWeek = pages.find(p => p.type === 'week' && p.dateStart && p.dateEnd && p.dateStart <= today && today <= p.dateEnd);
  if (todayWeek) { selectPlanPage(todayWeek.id); return; }
  const now = new Date();
  const monthTitle = MONTH_NAMES[now.getMonth()] + ' ' + now.getFullYear();
  const todayMonth = pages.find(p => p.type === 'month' && p.title === monthTitle);
  if (todayMonth) selectPlanPage(todayMonth.id);
}
...
function renderPlansNav() {
  ...
  if (!current) {
    actionsHtml = `
      ...
      <button class="plans-action ghost today-btn" onclick="goToToday()">📍 Сегодня</button>`;
  ...
  const chipsHtml = children.length ? `
    <div class="plans-chips">
      ${children.map(c => {
        const dateRow = (c.type === 'week' && c.dateStart && c.dateEnd)
          ? `<span class="plan-chip-dates">${c.dateStart} – ${c.dateEnd}</span>`
          : '';
        const prog = getPageProgress(c.id);
        const progRow = prog.total > 0 ? (() => {
          const pct = Math.round(prog.done / prog.total * 100);
          const cls = pct === 100 ? 'done' : pct > 0 ? 'progress' : '';
          return `<div class="plan-chip-progress ${cls}"><div class="plan-chip-progress-bar" style="width:${pct}%"></div><span class="plan-chip-progress-label">${prog.done}/${prog.total}</span></div>`;
        })() : '';
        const isToday = isTodayInWeek(c) || isTodayDay(c);
        const todayBadge = isToday ? '<span class="plan-chip-today">сегодня</span>' : '';
        return `
        <div class="plan-chip${isToday ? ' chip-today' : ''}" onclick="selectPlanPage('${c.id}')">
          <div class="plan-chip-body">
            <div class="plan-chip-title-row">
...
function renderPlansSidebar() {
  const container = document.getElementById('plans-tree');
  if (!container) return;

  const pages = loadPlanPages();
  const items = getItems('plans');

  function hasContent(pageId) {
    if (items.some(i => i.pageId === pageId)) return true;
    return pages.filter(p => p.parentId === pageId).some(c => hasContent(c.id));
  }
  ...
  function renderChildren(parentId, depth) {
    ...
    return children.map(c => {
      const active = currentPlanPageId === c.id ? ' active' : '';
      const filled = hasContent(c.id) ? ' filled' : '';
      const todayCls = (isTodayInWeek(c) || isTodayDay(c)) ? ' today' : '';
      const sub = renderChildren(c.id, depth + 1);
      return `<div class="pst-item depth-${depth}${active}${filled}${todayCls}" onclick="selectPlanPage('${c.id}')">
        <span class="pst-item-label">${escHtml(c.title)}</span>
        ${sub ? `<div class="pst-children">${sub}</div>` : ''}
      </div>`;
    }).join('');
  }
  ...
  if (!roots.length) {
    container.innerHTML = '<div class="pst-empty">Нет данных</div>';
    return;
  }

  let html = '';
  for (const y of Object.keys(yearMap).sort()) {
    html += `<div class="pst-year"><span class="pst-year-label">📅 ${y}</span><div class="pst-year-months">`;
    for (const m of yearMap[y]) {
      const active = currentPlanPageId === m.id ? ' active' : '';
      const filled = hasContent(m.id) ? ' filled' : '';
      const sub = renderChildren(m.id, 2);
      html += `<div class="pst-item depth-1${active}${filled}" onclick="selectPlanPage('${m.id}')">
        <span class="pst-item-label">${escHtml(m.title)}</span>
        ${sub ? `<div class="pst-children">${sub}</div>` : ''}
      </div>`;
    }
    html += '</div></div>';
...
function renderSidebarPlansTree() {
  const container = document.getElementById('sidebar-plans-tree');
  if (!container) return;
  ...
  function renderLevel(parentId, depth) {
    ...
    if (!children.length) return '';
    return children.map(c => {
      const active = currentPlanPageId === c.id ? ' spt-active' : '';
      const filled = hasContent(c.id) ? ' spt-filled' : '';
      const todayCls = (isTodayInWeek(c) || isTodayDay(c)) ? ' spt-today' : '';
      const sub = renderLevel(c.id, depth + 1);
      return `<div class="spt-item depth-${depth}${active}${filled}${todayCls}" onclick="selectPlanPage('${c.id}')">
        <span class="spt-label">${escHtml(c.title)}</span>
        ${sub ? `<div class="spt-children">${sub}</div>` : ''}
      </div>`;
    }).join('');
  }
  ...
  if (!roots.length) {
    container.innerHTML = '<div class="spt-empty">Нет данных</div>';
    return;
  }

  const yearMap = {};
  for (const m of roots) {
    const y = m.title.split(' ').pop();
    if (!yearMap[y]) yearMap[y] = [];
    yearMap[y].push(m);
  }
  ...
  let html = '';
  for (const y of Object.keys(yearMap).sort()) {
    html += `<div class="spt-year">
      <div class="spt-year-label">📅 ${y}</div>
      <div class="spt-year-body">`;
    for (const m of yearMap[y]) {
      const active = currentPlanPageId === m.id ? ' spt-active' : '';
      const filled = hasContent(m.id) ? ' spt-filled' : '';
      const sub = renderLevel(m.id, 2);
      html += `<div class="spt-item depth-1${active}${filled}" onclick="selectPlanPage('${m.id}')">
        <span class="spt-label">${escHtml(m.title)}</span>
        ${sub ? `<div class="spt-children">${sub}</div>` : ''}
      </div>`;
    }
    html += '</div></div>';
...
function renderSidebarCalendar() {
  ...
  // Дни с задачами планов
  const planItems = getItems('plans');
  const planPages = loadPlanPages();
  const datesWithPlans = new Set();
  for (const item of planItems) {
    if (!item.pageId) continue;
    // ищем страницу-день
    const page = planPages.find(p => p.id === item.pageId);
    if (!page) continue;
    // Страница-день содержит дату в названии: "Понедельник 30.06"
    const m = page.title.match(/(\d{2})\.(\d{2})/);
    if (m) {
      const key = `${m[2]}-${m[1]}`; // MM-DD
      datesWithPlans.add(`${_calYear}-${key}`);
    }
  }
  ...
  const monthName = MONTH_NAMES[_calMonth];
  const DOWS = ['Пн','Вт','Ср','Чт','Пт','Сб','Вс'];

  let cells = '';
  // Дни предыдущего месяца
  for (let i = dow - 1; i >= 0; i--) {
    cells += `<div class="sc-day other-month">${daysInPrev - i}</div>`;
  }
  // Дни текущего месяца
  for (let d = 1; d <= daysInMonth; d++) {
    const isToday = d === todayD && _calMonth === todayM && _calYear === todayY;
    const dd = d.toString().padStart(2,'0');
    const mm = (_calMonth+1).toString().padStart(2,'0');
    const hasP = datesWithPlans.has(`${_calYear}-${mm}-${dd}`);
    cells += `<div class="sc-day${isToday ? ' today' : ''}${hasP ? ' has-plans' : ''}">${d}</div>`;
  }
  // Дни следующего месяца
  const total = dow + daysInMonth;
  const tail = total % 7 === 0 ? 0 : 7 - (total % 7);
  for (let d = 1; d <= tail; d++) {
    cells += `<div class="sc-day other-month">${d}</div>`;
  }

  el.innerHTML = `
    <div class="sc-header">
      <button class="sc-nav" onclick="calPrev()">‹</button>
      <span class="sc-title">${monthName} ${_calYear}</span>
      <button class="sc-nav" onclick="calNext()">›</button>
    </div>
    <div class="sc-grid">
      ${DOWS.map(d => `<div class="sc-dow">${d}</div>`).join('')}
      ${cells}
    </div>
  `;
}
...
// ===== DRAGGABLE PLANS PANEL =====
(function initDraggablePanel() {
  const panel = document.getElementById('plans-right-sidebar');
  const handle = document.getElementById('prs-drag-handle');
  if (!panel || !handle) return;

  let dragging = false, startX = 0, startY = 0, startLeft = 0, startTop = 0;

  handle.addEventListener('mousedown', (e) => {
    if (e.target.classList.contains('prs-close')) return;
    dragging = true;
